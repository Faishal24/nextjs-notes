...
const EditNote = ({ data }) => {
  ...
  const handleSubmit = async (id) => {
    ...
    try {
      const res = await fetch(`/api/notes/${id}`, {
        method: "PATCH",
        body: JSON.stringify({ newTitle: title, newTag: tag, newContent: content }),
        headers: {
  ...
  return (
    <div>
      <h1 className="text-3xl font-semibold mb-7 text-primary">Edit Note</h1>
        ...
        <div className="pt-[1.25rem] sm:pt-0 sm:w-2/3">
          <div className="mb-5">
            ...
            <textarea
              ...
              className="w-full sm:min-h-40 p-3 border border-gray-300 rounded-md"
            ></textarea>
          </div>
        </div>
      </div>
